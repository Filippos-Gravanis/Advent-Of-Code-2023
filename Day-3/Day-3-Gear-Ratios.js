...
async function gearRatios() {
    ...
    for (let line = 0; line < input.length; line++) {
        ...
        while (charIndex < inputLine.length) {
            if (["1", "2", "3", "4", "5", "6", "7", "8", "9", "0"].includes(inputLine[charIndex])) {
                let PartNumber = false
                let endIndex = getNumberEndIndex(input[line], charIndex)
                if (line == 0) {
                    PartNumber = isPartNumber(charIndex, endIndex, ["", input[line + 1], inputLine])
                }
                else if (line == input.length - 1) {
                    PartNumber = isPartNumber(charIndex, endIndex, [input[line - 1], "", inputLine])
                }
                else {
                    PartNumber = isPartNumber(charIndex, endIndex, [input[line - 1], input[line + 1], inputLine])
                }
                if (PartNumber) {
...
async function getAllSymbolsIndex() {
    ...
    for (let line = 0; line < input.length; line++) {
        for (let charIndex = 0; charIndex < input[line].length; charIndex++) {
            if (!["1", "2", "3", "4", "5", "6", "7", "8", "9", "0"].includes(input[line][charIndex]) && input[line][charIndex] != ".") {
...
input = ["467..114...",
    ...
    allSumbolsIndexes.forEach(symbol=>{
        let numOfPartNumbers = 0
        let partNub
        allNumbersIndexes.forEach(number=>{
            if (number[0]== symbol[0]-1 || number[0]== symbol[0] || number[0]== symbol[0]+1 ){

                console.log(input[number[0]].slice(number[1][0],number[1][1]+1),"num")
...
function getNumberEndIndex(line, startIndex) {
    ...
    while (!found && endIndex < line.length) {
        if (["1", "2", "3", "4", "5", "6", "7", "8", "9", "0"].includes(line[endIndex])) {
            endIndex++
        }
        else {
            found = true
...
function isPartNumber(startIndex, endIndex, [aboveLine, belowLine, currentLine]) {
...
function getSymbolsIndex(line) {
    ...
    for (let charIndex = 0; charIndex < line.length; charIndex++) {
        if (!["1", "2", "3", "4", "5", "6", "7", "8", "9", "0"].includes(line[charIndex]) && line[charIndex] != ".") {
